    ...
    for index, job in enumerate(jobs):
        ...
        if 'few' in published_date:
            ...
            if unfamiliar_skill not in skills:
                f = open(f'web scrapper/1_output/output.txt', 'a')
                f.write(f"Company Name:{company_name.strip()} \n")
                f.write(f"Skill Required:{skills.strip()}\n")
...
for ids in contests:
    ...
    if "Global Round" in contest_name:
        contest_type = 1
    for i in range(total_no_of_question):
        data = data.append({'contest_type': contest_type, 'question_no': i+1, 'q_rating': problem_rating[i+1], 'accuracy': total_correct_sub_for_each_question[
                           i+1]/total_sub_for_each_question[i+1], 'avg_solving_time': avg_solve_time_for_each_question[i+1]}, ignore_index=True)
    # print(data)
    print(taken)
    if(taken % 10 == 0):
        data.to_csv('output{saved}.csv', index=False)
